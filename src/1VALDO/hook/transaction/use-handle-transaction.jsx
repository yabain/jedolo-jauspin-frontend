...
export const useHandleTransaction = (user, resetAfterSuccess, { onPending, onCancelled, onError, onSuccess, onstart }) => {
  ...
  const handleTransaction = async (formData) => {
    ...
    try {
      ...
      const response = await axios.post(`${HOST_FRONT_PROD}/transaction-payment`, dataTransaction, {
      ...
      const intervalId = setInterval(async () => {
        try {
          const statusResponse = await axios.get(`${HOST_FRONT_PROD}/transaction-payment/status/${transactionToken}`, {
            headers: { Authorization: `Bearer ${tokenUser()}` },
          });
