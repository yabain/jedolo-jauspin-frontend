...
export default function PostListView()
{
       ...
       const handleFilterByTownSelected = useCallback(
              ( event, newValue ) =>
              {
                     ...
                     const dataFilteredByProfil = filterByArguments( annonceFromStore, 'profile', [ selectedTab ] )
                     setFilteredPosts( filterByArguments( dataFilteredByProfil, 'city', [ newValue ] ) )
                     setSelectedTownTab( newValue )

                     // handleFilters( 'categorie', newValue );
              },

              [ filterByArguments, annonceFromStore, selectedTab ]
       );
                                          ...
                                          { [ 'Yaounde', 'Bafoussam', 'Douala', 'Bertoua' ].map( ( tab ) => (
                                                 <Tab
                                                        ...
                                                        icon={
                                                               ...
                                                               >
                                                                      {/* { tab === 'Yaounde' && posts.length } */ }
                                                                      { tab === 'Yaounde' && 100 }

                                                                      { tab === 'Bafoussam' && 55 }

                                                                      {/* { tab === 'Douala' && posts.filter( ( post ) => post.categorie === 'Standard' ).length } */ }
                                                                      { tab === 'Douala' && 37 }
                                                                      { tab === 'Bertoua' && 20 }
                                                               </Label>
                                                        }
