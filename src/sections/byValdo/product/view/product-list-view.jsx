...
import DialogALaUneBuy from 'src/1VALDO/components/annonces/aLaUne-buy/dialog-aLaUne-buy';
...
import { DataGrid, GridToolbarExport, GridActionsCellItem, GridToolbarContainer, GridToolbarQuickFilter, GridToolbarFilterButton, GridToolbarColumnsButton } from '@mui/x-data-grid';
...
import { RenderCellStock, RenderCellPrice, RenderCellPublish, RenderCellProduct, RenderCellCreatedAt, RenderCellNbrComment, RenderCellProductXs, RenderCellSponsored, RenderCellALaUne } from '../product-table-row';
...
export default function ProductListView({ toShow, clickFromProfile }) {
  ...
  const afterDelete = (deletedAnnonce) => {
    dispatch(setData(annoncesFunction.deleteAnnonceInArray(annonceFromStore, deletedAnnonce)));
  };
    ...
    // },
    {
      ...
      renderCell: (params) => (
        <RenderCellProductXs
          ...
          Sponsoriser={() => {
            buySponsor.onTrue();
            setAnnonceToSponsor(params.row);
          }}
          mettreALaUne={() => {
            aLaUneBuy.onTrue();
            setAnnonceClick(params.row);
          }}
          onEdit={() => {
      ...
      getActions: (params) => {
        if (clickFromProfile === undefined) {
          return [
            ...
            //        }
            // />,
            <GridActionsCellItem showInMenu icon={<Iconify icon="solar:pen-bold" />} label="Modifier" onClick={() => handleEditRow(params.row)} />,
            <GridActionsCellItem
              showInMenu
        ...
        return [<GridActionsCellItem showInMenu icon={<Iconify icon="solar:pen-bold" />} label="Voir" onClick={() => clickFromProfile(params.row)} />];
      },
    },
  ];

  const getTogglableColumns = () => columns.filter((column) => !HIDE_COLUMNS_TOGGLABLE.includes(column.field)).map((column) => column.field);

  return (
    <>
      {/* <DialogSponsorBuy 
        showDialog={buySponsor} 
        startPaiementSp={startPaimentSponsor} 
      /> */}
      {/* <DialogPaimentInProcessSponsorBuy 
        dataGet={annonceToSponsor} 
        showDialog={startPaimentSponsor} 
      /> */}
      <DialogALaUneBuy showDialog={aLaUneBuy} startPaiement={startPaimentALaUne} dataGet={annonceClick} />
      {/* <DialogPaimentInProcessALaUne 
        dataGet={annonceClick} 
        showDialog={startPaimentALaUne} 
      /> */}
      <Box
      ...
      >
        {user?.role === 'user' && clickFromProfile === undefined && (
          <CustomBreadcrumbs
            ...
            slots={{
              toolbar: () => (
                <>
                  ...
                  {canReset && <ProductTableFiltersResult filters={filters} onFilters={handleFilters} onResetFilters={handleResetFilters} results={dataFiltered.length} sx={{ p: 2.5, pt: 0 }} />}
                </>
              ),
      ...
      <DialogDeleteAnnonce
        showDialog={confirmOfDel}
        data={annonceToDel}
        handleAfterDelete={() => {
          afterDelete(annonceToDel);
        }}
      />
    </>
  );
